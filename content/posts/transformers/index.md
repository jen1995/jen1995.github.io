...
> 💡 **Why LayerNorm and not BatchNorm?** BatchNorm normalizes each coordinate using *batch* statistics: the mean and variance are computed over all examples at once. It has a well-known cost, not specific to text at all: an example's representation depends on its random batch neighbors, and at inference the batch statistics are replaced by running averages accumulated during training — the behavior at training and at inference slightly diverges. Computer vision lives with this cost successfully: there the statistics are stable from batch to batch, and the batch noise even helps as regularization. With sequences it is worse. Lengths differ — and although padding is easy to filter out of the statistics, the number of real tokens still jumps from batch to batch; moreover, the activation distributions depend heavily on which tokens landed in the batch — rare words produce outliers. As a result, batch statistics in a transformer are so noisy that the running averages become a poor estimate, and the train/inference mismatch starts to genuinely hurt quality — this is exactly what is shown experimentally in [Shen et al., 2020](https://arxiv.org/abs/2003.07845). Finally, in autoregressive generation the batch may consist of one example and one new token — there is simply nothing to compute statistics from. LayerNorm is free of all this because it is *per-token*: each vector — the representation of one token — is normalized on its own, over its own $d$ coordinates, identically at training and inference, at any sequence length and any batch size. Note that the two norms flip the axes: LayerNorm normalizes a token across its coordinates, while BatchNorm normalizes a coordinate across tokens — the statistics for the $j$-th coordinate are pooled over all tokens of all examples in the batch — $b \cdot n$ summands in the mean for a batch of $b$ sequences of length $n$, versus just the $d$ coordinates of a single vector in LayerNorm — so a BatchNorm'ed token inevitably depends on its neighbors, and no variant of it is ever per-token. Both axis conventions are easy to verify against [dniku/dl-norms](https://github.com/dniku/dl-norms) — minimal from-scratch implementations of the norm layers, tested to match PyTorch's: LayerNorm averages over the trailing `normalized_shape` dims, BatchNorm over "all dims except C". Note that LayerNorm has no universal "all dims except…" phrasing — it depends on `normalized_shape`: for the transformer input $(b, n, d)$ with `normalized_shape=(d,)` it averages over $d$ only, i.e. over all dims *except batch and position* — and it is precisely the "except position" part that makes it per-token.
>
> ![LayerNorm normalizes a row (a token); BatchNorm normalizes a column (a coordinate) across the batch](layernorm_vs_batchnorm.svg)
>
> The same question can be asked differently: why not one normalization over the whole sequence (over all $n \cdot d$ numbers)? First, its statistics would include future tokens — for the decoder that is an answer leak bypassing the causal mask, and during generation the statistics (and the representations of already-generated tokens) would change with every new token. Second, it does not solve the actual problem: the scale is evened out only *on average over the sentence*, while an individual outlier token with a huge norm remains an outlier — whereas it is precisely individual vectors that saturate the softmax in attention. Per-token normalization fixes each vector individually and opens no extra channels of interaction between tokens.
